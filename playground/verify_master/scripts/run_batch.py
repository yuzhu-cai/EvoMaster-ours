#!/usr/bin/env python3
"""Batch runner for VerifyMaster evaluation.

Runs the verify_master playground on selected BrowseComp dataset entries.
"""
...
def run_single_entry(entry_id: int, question: str, run_dir: Path, logger: logging.Logger) -> dict:
    """Run one dataset entry through the verify_master playground."""
    ...
    cmd = [
        ...
        "verify_master",
        "--config",
        str(project_root / "configs" / "verify_master" / "config_gpt.yaml"),
        "--task",
        question,
        "--run-dir",
        str(task_path),
...
def main():
    parser = argparse.ArgumentParser(description="Batch run verify_master on dataset entries")
