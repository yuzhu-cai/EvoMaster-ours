"""VerifyMaster playground.

A verification-centric multi-agent playground for BrowseComp.
The planner decides the next subtask, the executor uses web tools to gather
 evidence, and the verifier audits evidence quality before the planner commits
 to a final answer.
"""
...
ROLE_TOOL_NAMES = {
    "planner_agent": [],
    "executor_agent": ["think", "finish", "google_search", "web_fetch"],
    "verifier_agent": ["think"],
}

@register_playground("verify_master")
class VerifyMasterPlayground(BasePlayground):
    """Verification-centric BrowseComp playground."""

    def __init__(self, config_dir: Path = None, config_path: Path = None):
        if config_path is None and config_dir is None:
            config_dir = project_root / "configs" / "verify_master"
        super().__init__(config_dir=config_dir, config_path=config_path)
        self.logger = logging.getLogger(self.__class__.__name__)
        self.agents.declare("planner_agent", "executor_agent", "verifier_agent")
    ...
    def _load_dataset(self) -> dict[int, dict]:
        ...
        if not DATASET_PATH.exists():
            self.logger.warning("Dataset not found: %s", DATASET_PATH)
            return {}
    ...
    def _setup_tools(
        ...
        self.logger.info("Registered Browse tools: %s", self._browse_tool_names)
    ...
    @property
    def executor(self):
        return self.agents.executor_agent

    @property
    def verifier(self):
        return self.agents.verifier_agent

    def setup(self) -> None:
        self.logger.info("Setting up VerifyMaster playground...")
        self._setup_session()
        self._setup_agents()
        self.logger.info("VerifyMaster setup complete")
    ...
    def _setup_agents(self) -> None:
        ...
        for slot_name, agent in self.agents.items():
            ...
            # In VerifyMaster, each agent is created with its own tool registry.
            # `self.tools` points to the registry from the last-created agent, so
            # binding `web_fetch` there does not affect the executor's copy.
