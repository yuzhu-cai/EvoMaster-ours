    ...
    for attempt in range(1, SERPER_MAX_RETRIES + 1):
        ...
        try:
            ...
            if attempt < SERPER_MAX_RETRIES:
                ...
                continue
            return (
                f"[search_web] Timeout after {SERPER_MAX_RETRIES} attempts for '{query}' "
            ...
            if attempt < SERPER_MAX_RETRIES:
                time.sleep(SERPER_RETRY_DELAY_SECONDS * attempt)
                continue
            return f"[search_web] Error for '{query}' after {SERPER_MAX_RETRIES} attempts: {exc}"
        finally:
            if conn is not None:
                conn.close()
    return f"[search_web] Failed for '{query}': {last_error}"
