...
class GoogleScholarParams(BaseToolParams):
    """Search academic papers using google_scholar.py."""
...
class GoogleScholarTool(BaseTool):
    ...
    def execute(self, session, args_json: str) -> tuple[str, dict[str, Any]]:
        try:
            ...
            logger.info("google_scholar called (query_type=%s)", query_type)
            if self.external_script_path is not None:
                ...
                logger.info("google_scholar completed via external script: %s", self.external_script_path)
            ...
            logger.info("google_scholar completed via builtin")
            return ensure_text(result), {"tool": self.name, "mode": "builtin"}
        except Exception as exc:
            logger.error("google_scholar failed: %s", exc, exc_info=True)
...
def _scholar_one(query: str) -> str:
    key = os.getenv("SERPER_API_KEY", "").strip() or os.getenv("SERPER_KEY_ID", "").strip()
    if not key:
        return "[google_scholar] SERPER_API_KEY or SERPER_KEY_ID is not set."
