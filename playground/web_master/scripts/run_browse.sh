#!/bin/bash
# WebMaster evaluation workflow script
#
# Usage:
#   IDS="0-9" RUN_NAME="web_master_test" ./playground/web_master/scripts/run_browse.sh

set -e

IDS="${IDS:-0,1,3-8}"
RUN_NAME="${RUN_NAME:-web_master_test}"
RUN_WORKERS="${RUN_WORKERS:-8}"
EVAL_WORKERS="${EVAL_WORKERS:-4}"
DATA_JSON="${DATA_JSON:-playground/web_master/test/browsecomp_decrypted.json}"
...
echo "========================================" | tee -a "${LOG_FILE}"
echo "WebMaster Evaluation Workflow" | tee -a "${LOG_FILE}"
echo "========================================" | tee -a "${LOG_FILE}"
...
python "${PROJECT_ROOT}/playground/web_master/scripts/run_batch.py" \
    --json "${DATA_JSON}" \
    --lines "${IDS}" \
    --run-dir "${RUN_DIR}" \
    --workers "${RUN_WORKERS}"
...
# python "${PROJECT_ROOT}/playground/web_master/scripts/merge.py" \
#     --json "${DATA_JSON}" \
#     --run-dir "${RUN_DIR}" \
...
# python "${PROJECT_ROOT}/playground/web_master/scripts/eval.py" \
...
# python "${PROJECT_ROOT}/playground/web_master/scripts/summarize.py" \
