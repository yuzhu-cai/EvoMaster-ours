# RoboTwin hanging_mug Imitation Learning PRD Docs
...
**Goal:** Maximize the success rates of the `hanging_mug` tasks on the RoboTwin platform, and implement structured, reusable, and scalable automated improvements to the existing DP code. This task leverages a large-scale dataset of 1000 expert demonstrations.
...
#### Required dataset
- Use **only** `hanging_mug-demo_clean-10.zarr` (10 trajectories). It is fast and matches the real data format.
...
```python
debug_test(
    command="bash train.sh hanging_mug demo_clean 10 0 14 0",
    timeout=180,
    working_dir="policy/DP"
)
```
...
```python
debug_test(
    command="bash train.sh hanging_mug demo_clean 50 0 14 0",
    timeout=600,
    working_dir="policy/DP"
)
```
...
bash train.sh hanging_mug demo_clean_1000 1000 0 14 0
# Evaluation
bash eval.sh hanging_mug demo_clean_1000 demo_clean_1000 1000 0 0 30
```
...
### Task Specifics: `hanging_mug`
- **Complexity**: Bimanual coordination task requiring precise hanging of a mug onto a rack.
- **Action Space**: 14-dim (Dual arm setup with coordinated handoff).
- **Key Challenges**:
  - Bimanual coordination: Requires simultaneous control of two arms with different roles (grasping arm and hanging arm).
  - Handoff operation: The mug must be transferred from the left arm to the right arm at an intermediate position.
  - Precise alignment: The mug must be hung onto the rack's functional point with strict alignment constraints.
  - Multi-stage sequence: Grasp with left arm -> Lift -> Move to middle position -> Handoff to right arm -> Hang onto rack.
  - Spatial reasoning: Requires understanding the 3D relationship between mug, rack, and intermediate positions.
  - Object diversity: 10 different mug models are used.
