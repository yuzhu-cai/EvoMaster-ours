# RoboTwin open_laptop Imitation Learning PRD Docs
...
**Goal:** Maximize the success rates of the `open_laptop` tasks on the RoboTwin platform, and implement structured, reusable, and scalable automated improvements to the existing DP code. This task leverages a large-scale dataset of 1000 expert demonstrations.
...
#### Required dataset
- Use **only** `open_laptop-demo_clean-10.zarr` (10 trajectories). It is fast and matches the real data format.
- **Do not** use `demo_clean_1000`, and **do not** create your own test data.
...
```python
debug_test(
    command="bash train.sh open_laptop demo_clean 10 0 14 0",
    timeout=180,
    working_dir="policy/DP"
)
```
...
```python
debug_test(
    command="bash train.sh open_laptop demo_clean 50 0 14 0",
    timeout=600,
    working_dir="policy/DP"
)
```
...
- Large datasets (`demo_clean_1000`)
- Custom test data
...
bash train.sh open_laptop demo_clean_1000 1000 0 14 0
# Evaluation
bash eval.sh open_laptop demo_clean_1000 demo_clean_1000 1000 0 0 30
```
4. **Algorithmic Optimization (DP-specific):**
   - **Diffusion architecture:**
     - Adjust `down_dims` or `kernel_size` for articulated object manipulation. **Ensure shape alignment at skip connections.**
...
### Task Specifics: `open_laptop`
- **Complexity**: Single-arm manipulation task requiring opening a laptop lid through articulated object manipulation.
- **Action Space**: 14-dim (Dual arm setup, but typically uses one arm based on laptop orientation).
- **Key Challenges**:
  - Adaptive arm selection: The robot must choose the appropriate arm (left or right) based on the laptop's face orientation.
  - Articulated object manipulation: Requires understanding and controlling the laptop's joint to open the lid.
  - Sequential contact switching: The robot must grasp the laptop at different contact points (base and lid) to achieve the opening motion.
  - Multi-step operation: Involves grasping, maintaining contact, and rotating the lid through multiple iterations.
  - Object diversity: 11 different laptop models are used with varying initial configurations.
