...
1. **General training configuration**: ./robomimic/exps/paper_local/core/tool_hang/ph/low_dim/bc_rnn.json
...
```
# In ./robomimic/exps/paper_local/core/tool_hang/ph/low_dim/bc_rnn.json
...
```
This means the exeriment uses 'gmm' model, and you should modify Class 'BC_GMM' in ./robomimic/algo/bc.py. Please refer to bc.py for its connection to bc_rnn.json.
.
...
```
Use file_editor to view ./robomimic/algo/bc.py and ./robomimic/exps/paper_local/core/tool_hang/ph/low_dim/bc_rnn.json  (just example)
```

**Step 2: Make your improvements**
```
Use file_editor to modify specific sections: (just examples)
- Change learning rate from 1e-4 to 5e-5 in bc_rnn.json
- Modify loss calculation bc.py
...
```bash
python robomimic/scripts/train.py --config robomimic/exps/paper/core/tool_hang/ph/low_dim/bc_rnn.json
```
