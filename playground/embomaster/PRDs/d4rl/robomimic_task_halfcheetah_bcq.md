...
1. **General training configuration**: ./robomimic/exps/paper_local/d4rl/halfcheetah-expert-v2/bcq.json
2. **Model and loss function**: ./robomimic/algo/bcq.py

Note: Modify the relevant parts of the algorithm files as per the config content. For example:
```
# In ./robomimic/exps/paper_local/d4rl/halfcheetah-expert-v2/bcq.json
...
```
Use file_editor to view ./robomimic/algo/bcq.py and ./robomimic/exps/paper_local/d4rl/can/ph/low_dim/bcq.json  (just example)
```
...
```bash
python robomimic/scripts/train.py --config robomimic/exps/paper/d4rl/can/ph/low_dim/bcq.json
```
