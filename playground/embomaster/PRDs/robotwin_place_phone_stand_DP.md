...
#### Required dataset
- Use **only** `place_phone_stand-demo_clean-10.zarr` (10 trajectories). It is fast and matches the real data format.
...
```python
debug_test(
    command="bash train.sh place_phone_stand demo_clean 10 0 14 0",
    timeout=180,
    working_dir="policy/DP"
)
```
...
```python
debug_test(
    command="bash train.sh place_phone_stand demo_clean 50 0 14 0",
    timeout=600,
    working_dir="policy/DP"
)
```
...
bash train.sh place_phone_stand demo_clean_1000 1000 0 14 0
# Evaluation
bash eval.sh place_phone_stand demo_clean_1000 demo_clean_1000 1000 0 0 30
```
4. **Algorithmic Optimization (DP-specific):**
   - **Diffusion architecture:**
     - Adjust `down_dims` or `kernel_size` for precise placement coordination. **Ensure shape alignment at skip connections.**
...
- **Complexity**: Single-arm manipulation task requiring precise placement of a phone onto a phone stand.
- **Action Space**: 14-dim (Dual arm setup, but typically uses one arm based on phone position).
- **Key Challenges**:
  - Adaptive arm selection: The robot must choose the appropriate arm (left or right) based on the phone's initial position.
  - Precise alignment: The phone must be placed onto the stand's functional point with proper alignment constraints.
  - Multi-angle perception: Requires understanding desk layout from different camera angles to determine positions.
  - Object diversity: Multiple phone models (4 variants) and stand models (2 variants) are used.
